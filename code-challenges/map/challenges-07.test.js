...
const forLoopTwoToThe = arr => {
  ...
  for (const value in arr) {
    result.push(Math.pow(2, arr[value]));
  }
  return result;
};
...
const extractAbilities = arr => {
  let result = [];
  arr.map(ability => {
    result.push(Object.values(ability)[2].name);
  });
  return result;
};
...
const extractStats = arr => {
  ...
  arr.map(stat => {
    let resultName = [];
    let resultTotal = [];
    resultName.push(Object.values(stat)[0].name);
    resultTotal.push(Object.values(stat)[1] + Object.values(stat)[2]);
    result.push(new Result(resultName[0], resultTotal[0]));
  });
  return result;
};
